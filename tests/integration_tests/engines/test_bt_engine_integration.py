"""
Integration tests for bt engine

These tests verify the complete integration between bt engine
and other system components including EngineFactory, CLI commands,
and graceful handling of missing dependencies.
"""
...
try:
    import bt
    # Test if bt can actually be used (not just imported)
    # This catches deeper import failures in bt's dependencies
    bt.__version__  # This triggers the full import chain
    BT_AVAILABLE = True
...
def reset_factory_and_bt_state():
    """Reset EngineFactory and bt module state before each test for complete isolation"""
    
    # Import at test time to avoid import issues during collection
    from StrateQueue.engines import EngineFactory
    
    # STEP 1: Clear all engine-related modules from sys.modules
    modules_to_clear = [
        "StrateQueue.engines.bt_engine", 
        "src.StrateQueue.engines.bt_engine"
    ]
    ...
    # STEP 2: Reset the factory state completely  
    ...
    # CLEANUP: Reset everything again after the test
    try:
        EngineFactory._engines = {}
        EngineFactory._all_known_engines = {}
        EngineFactory._unavailable_engines = {}
    ...
    # Clear modules again to prevent state leakage
    for mod_name in modules_to_clear:
        if mod_name in sys.modules:
            del sys.modules[mod_name]
...
class TestBtEngineFactoryIntegration:
    ...
    def test_bt_engine_availability_check(self):
        ...
        # DIRECT APPROACH: Check bt_engine module directly to bypass factory caching issues
        # This ensures we get the true current state, not cached state from unit tests
        
        # Import the bt_engine module fresh
        import importlib
        import sys
        
        # Clear any cached bt_engine imports
        bt_modules = ["StrateQueue.engines.bt_engine", "src.StrateQueue.engines.bt_engine"]
        for mod_name in bt_modules:
            if mod_name in sys.modules:
                del sys.modules[mod_name]
        
        # Import fresh and check the actual BT_AVAILABLE state
        from StrateQueue.engines.bt_engine import BT_AVAILABLE as actual_bt_available, BtEngine
        
        # Now check the factory, but with a fresh factory state
        EngineFactory._engines = {}
        EngineFactory._all_known_engines = {}  
        EngineFactory._unavailable_engines = {}
        EngineFactory._initialized = False
        ...
        # Assertions based on actual bt availability, not test file's BT_AVAILABLE
        assert is_known, "bt engine should be known to EngineFactory"
        
        if actual_bt_available:
    ...
    def test_bt_engine_creation_without_dependencies(self):
        ...
        # DIRECT APPROACH: Check actual bt availability, not test file's BT_AVAILABLE
        import sys
        
        # Clear cached imports to get fresh state
        bt_modules = ["StrateQueue.engines.bt_engine", "src.StrateQueue.engines.bt_engine"]
        for mod_name in bt_modules:
            if mod_name in sys.modules:
                del sys.modules[mod_name]
        
        # Import fresh and check actual state
        from StrateQueue.engines.bt_engine import BT_AVAILABLE as actual_bt_available
...
class TestBtEngineDependencyManagement:
    ...
    def test_bt_engine_dependency_detection(self):
        ...
        # DIRECT APPROACH: Check actual bt availability to avoid cache issues
        import sys
        ...
        from StrateQueue.engines.bt_engine import BtEngine, BT_AVAILABLE as actual_bt_available
        
        # Test class-level dependency check
        deps_available = BtEngine.dependencies_available()
        assert deps_available == actual_bt_available
