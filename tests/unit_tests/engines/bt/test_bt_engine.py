...
@pytest.mark.skip(reason="Engine cannot be created without dependencies in current architecture")
...
def test_B1_create_engine_strategy_wraps_correctly(mock_bt_available, valid_bt_strategy):
    ...
    import sys
    from StrateQueue.engines.bt_engine import BtEngine, BtEngineStrategy
    
    engine = BtEngine()
    
    wrapped_strategy = engine.create_engine_strategy(valid_bt_strategy)
    
    # Use the freshly imported BtEngineStrategy for isinstance check
    assert isinstance(wrapped_strategy, BtEngineStrategy), f"Expected BtEngineStrategy, got {type(wrapped_strategy)}"
    assert wrapped_strategy.strategy_obj is valid_bt_strategy, f"Strategy object reference mismatch"
